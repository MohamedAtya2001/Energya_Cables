...
class ShowBeddingStandardController extends Controller
{
    ...
    public function showStandardBedding()
    {
        return view('Admin.ShowData.Standard.show_standard_bedding');
    }

    public function getStandardBedding(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($from != "" && $to != "") {
                $data = DB::table('beddingstandards')->where([
                ...
                $dataIdAsArray = DB::table('beddingstandards')->select('id')->where([
                    ['shift', 'LIKE', '%' . $request->filter['shift'] . '%'],
                    ['added_by', 'LIKE', '%' . $request->filter['added_by'] . '%'],
                    ['jopOrderNumber', 'LIKE', '%' . $request->filter['jopOrderNumber'] . '%'],
                    ['cableSize', 'LIKE', '%' . $request->filter['cableSize'] . '%'],
                ...
                $dataTime = DB::table('beddingstandardstimes')->whereIn('beddingstandards_id', $dataIdAsArray)->get();
    ...
    public function getRowToEditStandardBedding(Request $request)
    {
        if ($request->ajax()) {

            $data = DB::table('beddingstandards')->where('id', $request->rowId)->get()[0];
            $dataTime = DB::table('beddingstandardstimes')->where('beddingstandards_id', $request->rowId)->get()[0];

            return array($data, $dataTime);
        }
    }

    public function editStandardBedding(Request $request)
    {
        if ($request->ajax()) {
            ...
            $currentJopOrderNumber = DB::table('beddingstandards')->where('id', '=', $request->id)->value('jopOrderNumber');
            if ($currentJopOrderNumber != $request->jopOrderNumber[0]) {
                //Get Actuals That Linking With This Standard And Update JopOrderNumber
                $actuals = DB::table('beddingactuals')->where('jopOrderNumber', $currentJopOrderNumber)->update(['jopOrderNumber' => $request->jopOrderNumber[0]]);
                $actualstimes = DB::table('beddingactualstimes')->where('jopOrderNumber', $currentJopOrderNumber)->update(['jopOrderNumber' => $request->jopOrderNumber[0]]);
            }

            $rowDataBedding = BeddingStandard::where('id', '=', $request->id)
                ->update([
            ...
            $rowDataBeddingTime = BeddingStandardsTimes::where('beddingstandards_id', '=', $request->id)
                ->update([
            ...
            $beddingStandard = DB::table('beddingstandards')->where('id', $request->id)->first();
            $beddingStandardTime = DB::table('beddingstandardstimes')->where('beddingstandards_id', $request->id)->first();

            return array($beddingStandard, $beddingStandardTime);
        }
    }
}
