        ...
        success: function (data) {
            ...
            if (data == "Logout") {
                ...
                if ($(that).parents('.popUp').find(`.dropdownalert-menu .dropdownalert-item:not(.dropdownalert-menu .dropdownalert-item[updated="true"])`).length == 0) {
                    ...
                    $('.insulation .searchJopOrderNumberOfDrowing').find('.input').prop('disabled', false);
                    $('.insulation .searchJopOrderNumberOfDrowing').find('button').prop('disabled', false);
                }
...
//To Check Row
$('.insulation .input, .insulation textarea').blur(function () {
    let date = new Date().toLocaleString();
    let dataForm = {};
...
if (localStorage.getItem('dataOfInsulation') == null) {
    ...
    for (let i = 0; i <= 3; i++) {
        ...
        for (let j = 0; j < inputs_1.length; j++) {
            ...
            */
            dataOfInsulation_1[key] = ["", "", (j <= 13) ? true : false, false];
        }
        //To Know if user called data from alerts or not
    ...
    for (let i = 0; i <= 3; i++) {
        ...
        if (dataOfInsulation[i]['isAlert'][0]) {
            ...
            */
            ...
            $('.insulation .popUp').eq(i).attr('update-data-sheet-of-id', dataOfDrowing[i]['isAlert'][1]);
            //To Show Icon Of Clear Sheet
