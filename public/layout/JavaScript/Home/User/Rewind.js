...
$('.rewind form').submit(function (e) {
    ...
    let date = new Date().toLocaleString();
    dataForm['reasonOfRewind'] = [$(this).find('textarea').eq(0).val(), ($(this).find('textarea').eq(0).attr('data-time') != "") ? $(this).find('textarea').eq(0).attr('data-time') : date];
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data == "Logout") {
                ...
                SaveDataOfRewindLocalStorage(that, inputs);
            }
        },
        error: function (data) {
...
//To Save Data Of Rewind Sheets in LocalStorage for Refreash
let dataOfRewind = {};

if (localStorage.getItem('dataOfRewind') == null) {

    let inputs = $(`#Rewind_1 .rewindForm`).find('input:not(input[name="_token"])');
    let textarea = $(`#Rewind_1 .rewindForm`).find('textarea')[0];
    ...
    for (let j = 0; j < inputs.length; j++) {
        ...
        /* 
        dataOfRewind[0] => value of input
        dataOfRewind[1] => data-time of input
        dataOfRewind[2] => required
        */
        dataOfRewind[key] = ["", "", true];
    }


    // console.log(dataOfRewind);

    localStorage.setItem('dataOfRewind', JSON.stringify(dataOfRewind));

    deliverDataFromRewindLocalStorage();
...
function deliverDataFromRewindLocalStorage() {
    dataOfRewind = JSON.parse(localStorage.getItem('dataOfRewind'));
    ...
    for (let j = 0; j < inputs.length; j++) {
        let inputFromLocalStorage = dataOfRewind[$(inputs[j]).attr('name')];
...
function SaveDataOfRewindLocalStorage(element, inputs) {
    let dataOfRewind = JSON.parse(localStorage.getItem('dataOfRewind'));
    for (let j = 0; j < inputs.length; j++) {
        dataOfRewind[$(inputs[j]).attr('name')] = [$(inputs[j]).val(), $(inputs[j]).attr('data-time'), $(inputs[j]).prop('required')];
    }
    localStorage.setItem('dataOfRewind', JSON.stringify(dataOfRewind));
}
