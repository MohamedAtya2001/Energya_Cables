        ...
        success: function (data) {
            ...
            if (data == "Logout") {
                ...
                if ($(that).parents('.popUp').find(`.dropdownalert-menu .dropdownalert-item:not(.dropdownalert-menu .dropdownalert-item[updated="true"])`).length == 0) {
                    ...
                    $('.assembly .searchJopOrderNumberOfDrowing').find('.input').prop('disabled', false);
                    $('.assembly .searchJopOrderNumberOfDrowing').find('button').prop('disabled', false);
                }
...
//To Check Row
$('.assembly .input, .assembly textarea').blur(function () {
    $(this).find('button[type="submit"]').prop('disabled', true);
    $(this).find('.spinner').removeClass('d-none');
    let date = new Date().toLocaleString();
    let that = this;
...
if (localStorage.getItem('dataOfAssembly') == null) {
    ...
    for (let i = 0; i <= 2; i++) {
        ...
        for (let j = 0; j < inputs_1.length; j++) {
            ...
            */
            dataOfAssembly_1[key] = ["", "", (j <= 10) ? true : false, false];
        }
        //To Know if user called data from alerts or not
...
function deliverDataFromAssemblyLocalStorage() {
    ...
    for (let i = 0; i <= 2; i++) {
        ...
        let inputs = $(`#Assembly_${i + 1} .assemblyActual`).find('.input, textarea');
        ...
        if (dataOfAssembly[i]['isAlert'][0]) {
            ...
            */
            ...
            $('.assembly .popUp').eq(i).attr('update-data-sheet-of-id', dataOfDrowing[i]['isAlert'][1]);
            //To Show Icon Of Clear Sheet
