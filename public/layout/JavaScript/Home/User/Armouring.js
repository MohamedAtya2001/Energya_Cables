        ...
        success: function (data) {
            ...
            if (data == "Logout") {
                ...
                if ($(that).parents('.popUp').find(`.dropdownalert-menu .dropdownalert-item:not(.dropdownalert-menu .dropdownalert-item[updated="true"])`).length == 0) {
                    ...
                    $('.armouring .searchJopOrderNumberOfDrowing').find('.input').prop('disabled', false);
                    $('.armouring .searchJopOrderNumberOfDrowing').find('button').prop('disabled', false);
                }
...
//To Check Row
$('.armouring .input, .armouring textarea').blur(function () {
    let date = new Date().toLocaleString();
    let dataForm = {};
...
if (localStorage.getItem('dataOfArmouring') == null) {
    ...
    for (let i = 0; i <= 1; i++) {
        ...
        for (let j = 0; j < inputs_1.length; j++) {
            ...
            */
            dataOfArmouring_1[key] = ["", "", (j <= 7) ? true : false, false];
        }
        //To Know if user called data from alerts or not
    ...
    for (let i = 0; i <= 1; i++) {
        ...
        if (dataOfArmouring[i]['isAlert'][0]) {
            ...
            */
            ...
            $('.armouring .popUp').eq(i).attr('update-data-sheet-of-id', dataOfDrowing[i]['isAlert'][1]);
            //To Show Icon Of Clear Sheet
