        ...
        success: function (data) {
            ...
            if (data == "Logout") {
                ...
                if ($(that).parents('.popUp').find(`.dropdownalert-menu .dropdownalert-item:not(.dropdownalert-menu .dropdownalert-item[updated="true"])`).length == 0) {
                    ...
                    $('.sheathing .searchJopOrderNumberOfDrowing').find('.input').prop('disabled', false);
                    $('.sheathing .searchJopOrderNumberOfDrowing').find('button').prop('disabled', false);
                }
...
//To Check Row
$('.sheathing .input, .sheathing textarea').blur(function () {
    let date = new Date().toLocaleString();
    let dataForm = {};
...
if (localStorage.getItem('dataOfSheathing') == null) {
    ...
    for (let i = 0; i <= 3; i++) {
        ...
        for (let j = 0; j < inputs_1.length; j++) {
            ...
            */
            dataOfSheathing_1[key] = ["", "", (j <= 13) ? true : false, false];
        }
        //To Know if user called data from alerts or not
    ...
    for (let i = 0; i <= 3; i++) {
        ...
        if (dataOfSheathing[i]['isAlert'][0]) {
            ...
            */
            ...
            $('.sheathing .popUp').eq(i).attr('update-data-sheet-of-id', dataOfDrowing[i]['isAlert'][1]);
            //To Show Icon Of Clear Sheet
