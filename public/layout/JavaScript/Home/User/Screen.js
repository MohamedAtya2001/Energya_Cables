        ...
        success: function (data) {
            ...
            if (data == "Logout") {
                ...
                if ($(that).parents('.popUp').find(`.dropdownalert-menu .dropdownalert-item:not(.dropdownalert-menu .dropdownalert-item[updated="true"])`).length == 0) {
                    ...
                    $('.screen .searchJopOrderNumberOfDrowing').find('.input').prop('disabled', false);
                    $('.screen .searchJopOrderNumberOfDrowing').find('button').prop('disabled', false);
                }
...
//To Check Row
$('.screen .input, .screen textarea').blur(function () {
    let date = new Date().toLocaleString();
    let dataForm = {};
...
if (localStorage.getItem('dataOfScreen') == null) {
    ...
    for (let i = 0; i <= 2; i++) {
        ...
        for (let j = 0; j < inputs_1.length; j++) {
            ...
            */
            dataOfScreen_1[key] = ["", "", (j <= 6) ? true : false, false];
        }
        //To Know if user called data from alerts or not
    ...
    for (let i = 0; i <= 2; i++) {
        ...
        if (dataOfScreen[i]['isAlert'][0]) {
            ...
            */
            ...
            $('.screen .popUp').eq(i).attr('update-data-sheet-of-id', dataOfDrowing[i]['isAlert'][1]);
            //To Show Icon Of Clear Sheet
