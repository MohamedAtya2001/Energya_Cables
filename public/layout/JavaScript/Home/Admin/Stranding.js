        ...
        success: function (data) {
            ...
            if (data == "Updated") {
                ...
                if ($(that).parents('.popUp').find(`.dropdownalert-menu .dropdownalert-item:not(.dropdownalert-menu .dropdownalert-item[updated="true"])`).length == 0) {
                    ...
                    $('.stranding .searchJopOrderNumberOfStranding').find('.input').prop('disabled', false);
                    $('.stranding .searchJopOrderNumberOfStranding').find('button').prop('disabled', false);
                }
    ...
    for (let i = 0; i <= 9; i++) {
        ...
        for (let j = 0; j < inputs_1.length; j++) {
            ...
            */
            ...
            j <= 13 => For label
            j == 16 => For Shape Actual
            */
            dataOfStranding_1[key] = ["", "", (j <= 12 || j == 13 || j == 16) ? true : false, false];
        }
        //To Know if user called data from alerts or not
...
function deliverDataFromStrandingLocalStorage() {
    ...
    for (let i = 0; i <= 9; i++) {
        ...
        let inputs = $(`#Stranding_${i + 1} .strandingActual`).find('.input, textarea');
