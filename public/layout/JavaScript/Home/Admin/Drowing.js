    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data == "Updated") {
                ...
                $('.searchJopOrderNumberOfDrowing').find('.input');
                $('.searchJopOrderNumberOfDrowing').find('button');
...
if (localStorage.getItem('dataOfDrowing') == null) {
    ...
    for (let i = 0; i <= 3; i++) {
        ...
        for (let j = 0; j < inputs_1.length; j++) {
            ...
            j == 8 => For label
            */
            dataOfDrowing_1[key] = ["", "", (j <= 7 || j == 8) ? true : false, false];
        }
        //To Know if user called data from alerts or not
...
function SaveDataOfDrowingLocalStorage(element, inputs, isAlert = false, idOfAlert = null) {
    let dataOfDrowing = JSON.parse(localStorage.getItem('dataOfDrowing'));
    ...
    dataOfDrowing[item]['isAlert'] = [isAlert, idOfAlert];
    localStorage.setItem('dataOfDrowing', JSON.stringify(dataOfDrowing));
}
