        ...
        success: function (data) {
            ...
            if (data == "Updated") {
                ...
                if ($(that).parents('.popUp').find(`.dropdownalert-menu .dropdownalert-item:not(.dropdownalert-menu .dropdownalert-item[updated="true"])`).length == 0) {
                    ...
                    $('.bedding .searchJopOrderNumberOfBedding').find('.input').prop('disabled', false);
                    $('.bedding .searchJopOrderNumberOfBedding').find('button').prop('disabled', false);
                }
...
if (localStorage.getItem('dataOfBedding') == null) {
    ...
    for (let i = 0; i <= 1; i++) {
        ...
        for (let j = 0; j < inputs_1.length; j++) {
            ...
            j == 14 => For label
            */
            dataOfBedding_1[key] = ["", "", (j <= 13 || j == 14) ? true : false, false];
        }
        //To Know if user called data from alerts or not
    ...
    for (let i = 0; i <= 1; i++) {
        ...
        if (dataOfBedding[i]['isAlert'][0]) {
            ...
            */
            ...
            $('.bedding .popUp').eq(i).attr('update-data-sheet-of-id', dataOfBedding[i]['isAlert'][1]);
            //To Show Icon Of Clear Sheet
