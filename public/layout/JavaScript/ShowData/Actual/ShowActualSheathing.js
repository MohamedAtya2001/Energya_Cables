...
$("#FilterValue").blur(function () {
    filterOfActualSheathing[$("#FilterName").val()] = $(this).val();
    if ($(this).val() != '') {
...
$("#reportrange").on('apply.daterangepicker, hide.daterangepicker', function (ev, picker) {
    ...
    filterOfActualSheathing["periodOfTime"]['start'] = $("#reportrange span").attr('data-rangedatestart');
    filterOfActualSheathing["periodOfTime"]['end'] = $("#reportrange span").attr('data-rangedateend');
...
$("#reportrange").on('cancel.daterangepicker', function () {
    filterOfActualSheathing["periodOfTime"]['start'] = "";
    filterOfActualSheathing["periodOfTime"]['end'] = "";
...
$("#FilterName").change(function () {
    if ($(this).val() == "periodOfTime") {
        ...
        $("#reportrange span").text(`${filterOfActualSheathing["periodOfTime"]['start']} - ${filterOfActualSheathing["periodOfTime"]['end']}`);
        ...
        if ($('#FilterName').find(`option[value="notes"]`).hasClass('filtered')) {
            filterOfActualSheathing['notes'] = false;
            $('#FilterName').find(`option[value="notes"]`).removeClass('filtered');
        } else {
            filterOfActualSheathing['notes'] = true;
            $('#FilterName').find(`option[value="notes"]`).addClass('filtered');
        }
    } else {
        $(this).attr('data-last-selected', $(this).val());
        $("#FilterValue").val(filterOfActualSheathing[$(this).val()]);
        $('#reportrange').css('display', 'none');
...
function sheetsTypeFilter(that) {
    ...
    filterOfActualSheathing['sheetsType'] = ($(that).prop('checked')) ? $(that).attr('name') : $('#SheetsType input').not(that).attr('name');
...
function statusFilter(that) {
    ...
    filterOfActualSheathing['status']['hold'] = $('input#Hold').prop('checked');
    filterOfActualSheathing['status']['pass'] = $('input#Pass').prop('checked');
...
function GetData(that, moreData = false) {
    ...
    filterOfActualSheathing['limit'] = parseInt($("#Limit").attr('data-count-rows'));
    if (filterOfActualSheathing['sheetsType'] == "uncomplete") {
        $("#Limit").attr('data-count-rows', 25);
        filterOfActualSheathing['limit'] = 25;
    }
    dataForm['filter'] = filterOfActualSheathing;
    $.ajax({
        'url': 'Actual',
        'type': 'POST',
        'data': dataForm,
        'success': function (data) {
            ...
            if (countOfActualsRows > filterOfActualSheathing['limit']) {
            ...
            for (let i = 0; i < actualRows.length; i++) {
                ...
                $('#Data .actual .table tbody').append(`<tr data-id="${actualRows[i].id}">
                    <th>${(filterOfActualSheathing['limit'] - 24) + i}</th>
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(inputs).not(` #Edit input[name="dimBefore2"],
                            ...
                            #Edit textarea`).prop('required', (filterOfActualSheathing['sheetsType'] == 'complete'));

            // To Preper Data Of SheathingActual
            let sheathingActual_id = actualRow['id'];
            ...
            delete actualTimeRow['sheathingactuals_id'];
            ...
            $("#Edit form").attr('data-id', sheathingActual_id);
            $('#Edit').fadeIn(500);
...
$('.jopOrderNumber form').submit(function (e) {
    ...
    clearFilter();
    $("#Limit").attr('data-count-rows', 25);
    filterOfActualSheathing['jopOrderNumber'] = $('.jopOrderNumber .input').val();
    GetData(this);
});
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (Array.isArray(data)) {
                ...
                $('#Edit').get('0').scrollTop += $($(that).find('textarea[name="ovalityActual1"]')).offset().top - 20;
        ...
        'success': function (data) {
            ...
            for (let i = 0, j = 1; i < actualRows.length; i++, j++) {
                ...
                if (j % 3 != 0 && j != actualRows.length) {
                    ...
                    let actualTable = `<div class="actual my-3 sheathing">
