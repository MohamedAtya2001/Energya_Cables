...
$("#FilterValue").blur(function () {
    filterOfStandardLead[$("#FilterName").val()] = $(this).val();
    if ($(this).val() != '') {
...
$("#reportrange").on('apply.daterangepicker, hide.daterangepicker', function (ev, picker) {
    ...
    filterOfStandardLead["periodOfTime"]['start'] = $("#reportrange span").attr('data-rangedatestart');
    filterOfStandardLead["periodOfTime"]['end'] = $("#reportrange span").attr('data-rangedateend');
...
$("#reportrange").on('cancel.daterangepicker', function () {
    filterOfStandardLead["periodOfTime"]['start'] = "";
    filterOfStandardLead["periodOfTime"]['end'] = "";
...
$("#FilterName").change(function () {
    if ($(this).val() == "periodOfTime") {
        $("#reportrange span").text(`${filterOfStandardLead["periodOfTime"]['start']} - ${filterOfStandardLead["periodOfTime"]['end']}`);
        $('#reportrange').css('display', 'block');
        $("#FilterValue").css('display', 'none');
    } else {
        $("#FilterValue").val(filterOfStandardLead[$(this).val()]);
...
function GetData(that = null, moreData = false) {
    ...
    filterOfStandardLead['limit'] = parseInt($("#Limit").attr('data-count-rows'));
    dataForm['filter'] = filterOfStandardLead;
    ...
    $.ajax({
        ...
        'success': function (data) {
            ...
            if (countOfStandardsRows > filterOfStandardLead['limit']) {
            ...
            for (let i = 0; i < lead.length; i++) {
                ...
                $('#Data .actual .table tbody').append(`<tr data-id="${lead[i].id}">
                    <th>${(filterOfStandardLead['limit'] - 24) + i}</th>
                    <td>${lead[i].created_at.split(" ").join("<br>")}</td>
                    <td>${romanShift(lead[i].shift)}</td>
                    <td>${lead[i].added_by}</td>
                    <td>${lead[i].jopOrderNumber} <abbr title="${leadTime[i].jopOrderNumber_time}"><i class="fas fa-stopwatch"></i></abbr></td>
                    <td>${lead[i].size} <abbr title="${leadTime[i].size_time}"><i class="fas fa-stopwatch"></i></abbr></td>
                    <td>${lead[i].description} <abbr title="${leadTime[i].description_time}"><i class="fas fa-stopwatch"></i></abbr></td>
                    <td>${lead[i].volt} <abbr title="${leadTime[i].volt_time}"><i class="fas fa-stopwatch"></i></abbr></td>
                    <td>${lead[i].thicknessMinStandard} <abbr title="${leadTime[i].thicknessMinStandard_time}"><i class="fas fa-stopwatch"></i></abbr></td>
                    <td>${lead[i].thicknessNomStandard} <abbr title="${leadTime[i].thicknessNomStandard_time}"><i class="fas fa-stopwatch"></i></abbr></td>
                    <td>${lead[i].thicknessMaxStandard} <abbr title="${leadTime[i].thicknessMaxStandard_time}"><i class="fas fa-stopwatch"></i></abbr></td>
                    <td>${lead[i].dimAfter} <abbr title="${leadTime[i].dimAfter_time}"><i class="fas fa-stopwatch"></i></abbr></td>
                    <td>${lead[i].weightStandard} <abbr title="${leadTime[i].weightStandard_time}"><i class="fas fa-stopwatch"></i></abbr></td>
                    <td>${lead[i].updated_by} <abbr title="${lead[i].updated_at}"><i class="fas fa-stopwatch ${(lead[i].updated_by == '') ? 'd-none' : ''}"></i></abbr></td>
                    <td class="option">
                    <button class="btn btn-primary" onclick="editRow(${lead[i].id}, this)">
...
function editRow(rowId, that) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(that).find('.spinner').addClass('d-none');
            $(that).find('.bug').addClass('d-none');
            let leadStandard = data[0];
            let leadStandardTime = data[1];
            let inputs = $('#Edit form').find('.input');

            // To Preper Data Of StrandingStandard
            let leadStandard_id = leadStandard['id'];

            // To Preper Data Of LeadStandard
            delete leadStandard['id'];
            delete leadStandard['added_by'];
            delete leadStandard['shift'];
            delete leadStandard['updated_by'];
            delete leadStandard['created_at'];
            delete leadStandard['updated_at'];

            delete leadStandardTime['id'];
            delete leadStandardTime['leadstandards_id'];
            delete leadStandardTime['added_by'];
            delete leadStandardTime['shift'];
            delete leadStandardTime['updated_by'];
            delete leadStandardTime['created_at'];
            delete leadStandardTime['updated_at'];

            for (let key in leadStandard) {
                let currentInput = $(inputs).filter(`[name="${key}"]`);
                currentInput.val(leadStandard[key]);
                currentInput.attr('data-time', leadStandardTime[key + '_time']);
            }


            $('body').css('overflow', 'hidden');
            $("#Edit form").attr('data-id', leadStandard_id);
            $('#Edit').fadeIn(500);
